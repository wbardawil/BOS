...
export function gradeEvidence(input: EvidenceGradingInput): EvidenceGradingResult {
  ...
  /* v8 ignore start — defensive guard carried from bds-OS: unreachable while
     targetLevel = min(current_level + 1, 5); the edge function dropped it (DRIFT.md) */
...
function extractRelevantExcerpt(description: string, criterion: string): string | null {
  ...
  /* v8 ignore next 2 — only called for met criteria, so some sentence always contains a keyword */
  return null;
}
