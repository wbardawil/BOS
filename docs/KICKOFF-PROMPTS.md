...
### PROMPT 5a — Strategy vertex schema ✅ DONE (commit 0127a77)
> Migration 0003 (possibilities + aspirations + strategic_choices + assumptions, RLS-policied, adversarial suite extended) shipped. The rest of the original Prompt 5 (workspace CRUD UI) moves to PROMPT 9 — **the loop comes first** (SPEC-ADDENDUM §7): everything differential depends on live data; a cockpit without the loop is the dead dashboard the market already abandoned.

### PROMPT 6 — LOOP part 1: conectar (connections + CSV fallback + sync)
> Read SPEC-ADDENDUM §1.1/§3. Migration: `connections`, `metric_sources`, extend `metrics`/`metric_points` with provenance + freshness; RLS + adversarial specs. Build: (1) one REAL connector end-to-end (founder picks: Odoo or GoHighLevel) syncing on Inngest cron into metric_points; (2) CSV template pipeline (upload → validate → metric_points) with 3 templates: caja semanal, ventas por SKU, nómina/rotación — the fallback IS the product for $1–10M companies; (3) connector health: healthy/degraded/down, explicit stale propagation (a metric with a down source is marked, never extrapolated); (4) Conectores & datos screen per wireframe. Definition of done: real or fictional-CSV data flows into metric_points with provenance; pnpm test:rls green.

### PROMPT 7 — LOOP part 2: auditar (BDS-82 pack + inferred assessment + deterministic signals)
> (1) Extract BDS-82 from bds-OS (`supabase/seed.sql` + `data/*.json`: 82 practices, 8 areas, 410 rubrics, P&L metadata) as pack #2 of @bos/methodology — behavior-preserving, founder-ratified IP, integrity tests like the CDIO bank's. (2) `methodology_packs` migration. (3) Inferred assessment: map connected metrics to practice rubrics, pre-fill what data already answers, generate the 12–20 gap questions only (SPEC-ADDENDUM H2). Maturity NEVER auto-moves from data — agent proposes re-assessment, human answers (law). (4) Signals engine v1 in @bos/scoring (PURE): rule = threshold + trend + assumption linkage → `signals` rows; LLM (via @bos/ai) only writes the so-what; root cause stored as hypothesis + confidence. 100% coverage on the pure engine. Definition of done: fictional workspace shows ≥2 signals with visible rules; gap-question count ≤20.

### PROMPT 8 — LOOP part 3: priorizar (issues/opportunities + bandeja con detalle + undo)
> (1) Migrations: `issues`, `opportunities` (origin-tracked), promotion to initiative only by displacing one (hard WIP) and anchoring to a choice. (2) Prioritization: BDS-82 metadata (pnl_impact, speed_to_impact, dependency_score, risk_floor) + lifecycle engine (@bos/scoring, already extracted) ranks the queue. (3) Approval tray done RIGHT per the audited wireframe: every `agent_runs.awaiting_approval` opens a detail panel (exact outbound text, sources, reject-with-note) and approve carries a 30s undo before execution. (4) Agent figures rendered with "estimación" tag + bands. Definition of done: issue → prioritized → promoted → initiative flow works in browser; no outbound path skips the tray; pnpm test:rls green.

### PROMPT 9 — El lunes demostrable (cadencia + agenda + readout + triangle CRUD)
> The wedge demo (SPEC-ADDENDUM §1.2, simulación wireframe). (1) Workspace shell + triangle home + CRUD from the original Prompt 5 (aspirations versioned, choices, assumptions lifecycle, initiatives w/ orphan flag, capabilities/metrics). (2) Cadence sessions + decision log (original Prompt 6 scope). (3) Agenda prep by agent: signals + OKRs + open challenges → agenda where every point knows its origin; readout export (Markdown→PDF, Supabase Storage). (4) Advisor v0 with prompt-cached workspace context, zod tools to log decisions/challenges, citations. Definition of done: the founder runs a full real Monday — auto-fed scorecard → signals → agenda → decision → readout — without PowerPoint, and prefers it.

### PROMPT 10 — Phase 1 hardening (run after the demand-test gate passes)
> Unchanged scope from the original Prompt 7: onboarding (day-zero = the loop in miniature, honest empty states per wireframe), Stripe org-based subscription + metered usage with caps (SPEC-ADDENDUM §5: per-org pricing, frontline free), Upstash rate limiting, audit log, M2 diagnostics trending. Definition of done per TECH-SPEC Phase 1 gate; schema/auth changes get a second-voice review before merge.

### PROMPTS 11+ — Phase 2/3 (write fresh plans when you get there)
> Challenge Loop engine, Coherence v1, coaches por persona (3 data classes with the strictest RLS in the system — `coach_messages` readable by NOBODY but the person), Artifact Engine v1 (white-label), DECIDE module v1 + corpus, V2 working agents, real RAG (pgvector), V3 cadence agent + Executive tier, saved_views analyst v1 (curated catalog — free prompt is v2, LLM-to-SQL never). Each begins in plan mode against SPEC §10 / TECH-SPEC §3 / SPEC-ADDENDUM — do not pre-write these prompts now.
...
2. ✅ Prompts 0–5a are in main. **The loop (6→7→8) runs before any cockpit UI** — inverted 2026-07-17 (SPEC-ADDENDUM §7): first connect→audit→prioritize, then the demonstrable Monday (9), then hardening (10).
3. Prompt 9 is the dogfood deadline: the founder's real Monday session is the acceptance test, nothing else.
4. The demand test (SPEC §10 Phase 0b) is founder work, runs in parallel — now coach-led with existing consulting clients (SPEC-ADDENDUM §6). **Prompt 10 does not run until the demand gate passes.**
5. Every session ends with: build green, tests green, RLS suite green, commit with a why-message. No exceptions — the prototype carries real client data early.
6. Architecture laws apply to every prompt: signals deterministic, maturity moves only by human answer, approvals with detail+undo, agent figures labeled estimates, engines pure, Ley 7 (read progress ≠ manage tasks).
